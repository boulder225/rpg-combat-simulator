...
must_haves:
  truths:
    ...
    - "--verbose flag help text updated; single-run always verbose for backward compat"
  artifacts:
...
@.planning/phases/05-spells-conditions-resume/05-02-SUMMARY.md
@src/io/logger.py
@src/simulation/simulator.py
...
**2. CLI changes (`run.py` and `src/cli/batch_args.py`):**

The `--verbose` flag in batch_args.py currently says "ignored in single combat mode". Fix this so single-run mode respects `--verbose`, defaulting to True for backward compatibility.

In `run_single_combat()`, add a `verbose` parameter defaulting to True:
```python
def run_single_combat(creatures, agent, seed=None, terrain=None, verbose=True):
```

Pass it through to `run_combat`:
```python
final_state, logger = run_combat(creatures, agent, seed=seed, verbose=verbose, terrain=terrain)
```

In `main()`, pass `args.verbose` when calling single combat. But for backward compat (single-run defaults to verbose), change the CLI to default verbose to True in single mode. The cleanest approach: in `main()`, compute the effective verbose flag:
```python
# Single combat defaults to verbose=True unless --no-verbose is used
verbose = args.verbose if args.runs is not None else True
```

Then pass `verbose=verbose` to `run_single_combat()`.

Alternatively (simpler): just pass `verbose=True` for single mode since there's no `--no-verbose` flag, and note that `--verbose` currently only affects batch mode. The existing `--verbose` help text should be updated from "ignored in single combat mode" to "Enable verbose output (always on in single combat mode)".

In `src/cli/batch_args.py`, update the `--verbose` help text:
```python
parser.add_argument(
    "--verbose",
    action="store_true",
    help="Enable verbose output (always on in single combat mode)"
)
```
  </action>
