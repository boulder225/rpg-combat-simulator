...
- Do NOT call `random.seed(seed)` if resuming — the RNG state was already restored by `load_checkpoint()`
...
**IMPORTANT:** batch_args.py has dead code after line 201 (duplicate argument definitions after the first `return` statement). Before adding new arguments:
1. Delete the dead code block (lines 202-253, everything after the first `return BatchArgs(...)` statement)
2. Add the new arguments BEFORE the `return` statement (before line 188), alongside the existing `parser.add_argument` calls
...
Add to argument parser (BEFORE the `parsed = parser.parse_args(args)` call on line 176):
```python
parser.add_argument(
    "--resume",
    type=str,
